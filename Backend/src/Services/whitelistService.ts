...
export default class WhitelistService {
  ...
  static async getWhitelist(proj?: object): Promise<any> {
    const whitelist = await whitelistCollection.findOne(
      { whitelistID: _whitelistID }, proj)
    if (!whitelist) {
  ...
  static async addToWhitelist(IP: string): Promise<any> {
    ...
    if (_whitelist) {
      ...
      await whitelistCollection.updateOne(
        { whitelistID: _whitelistID }, { $set: { 'IPs': _whitelist.IPs } }
      );
    }
    return _whitelist
  }
